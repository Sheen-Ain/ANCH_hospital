...
function generateToken(): void
{
    ...
    try {
        ...
        // Insert payment — amount = sum of all selected charges (or doctor fee if none)
        ...
        // Insert per-patient charges breakdown
...
/**
 * getCharges — returns all active charge types from extra_charges table.
 * OPD is always first (is_opd=1). Amount for OPD is 0 (dynamic at runtime = doctor fee).
 */
...
/**
 * addCharge — receptionist can add new charge types for future use.
 * POST: name*, amount*
 * Validation: name must be unique (case-insensitive), not 'OPD'.
 */
function addCharge(): void
{
    ...
    if (strlen($name) > 100) jsonResponse(false, 'Charge name must be 100 characters or less.');
    if (!is_numeric($amount) || (float)$amount < 0) jsonResponse(false, 'Amount must be a valid number (0 or more).');
    ...
    $existing = Database::fetchOne(
        "SELECT id FROM extra_charges WHERE LOWER(name) = LOWER(?)",
        [$name]
    );
    if ($existing) {
        jsonResponse(false, "A charge named \"{$name}\" already exists.");
    }
    ...
    logActivity(getCurrentUserId(), 'charge_added', ['id' => $id, 'name' => $name, 'amount' => $amount]);
