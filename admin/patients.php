...
@media(max-width:480px)  { .stats-grid { grid-template-columns:1fr 1fr; gap:.625rem; } }
...
.stat-card.sc-blue::after   { background:var(--color-primary); }
.stat-card.sc-green::after  { background:#059669; }
.stat-card.sc-amber::after  { background:#d97706; }
.stat-card.sc-slate::after  { background:#64748b; }
...
.sc-blue  .stat-icon { background:var(--color-primary-lt); color:var(--color-primary); }
.sc-green .stat-icon { background:#d1fae5; color:#059669; }
.sc-amber .stat-icon { background:#fef3c7; color:#d97706; }
.sc-slate .stat-icon { background:#f1f5f9; color:#64748b; }
...
.stat-label { font-size:11px; color:var(--color-text-muted); margin-top:2px; font-weight:500; }
.stat-sub   {
    font-size:11px; font-weight:600; margin-top:5px;
    display:inline-flex; align-items:center; gap:4px;
    padding:2px 7px; border-radius:20px;
}
.sc-blue  .stat-sub { background:var(--color-primary-lt); color:var(--color-primary); }
.sc-green .stat-sub { background:#d1fae5; color:#059669; }
.sc-amber .stat-sub { background:#fef3c7; color:#d97706; }
.sc-slate .stat-sub { background:#f1f5f9; color:#64748b; }
...
.page-btns-wrap { display:flex; align-items:center; gap:.25rem; flex-wrap:wrap; }
.page-btn {
...
.page-btn:hover:not(:disabled) { background:var(--color-primary-lt); border-color:var(--color-primary); color:var(--color-primary); }
...
.cds-trigger:hover,.cds-trigger.open { border-color:var(--color-primary); }
.cds-caret {
    position:absolute; right:10px; top:50%; transform:translateY(-50%);
    color:var(--color-text-faint); font-size:11px; pointer-events:none;
    transition:transform .2s;
}
.cds-trigger.open .cds-caret { transform:translateY(-50%) rotate(180deg); }
.cds-pill {
    display:inline-flex; align-items:center; gap:4px; font-size:11px; font-weight:600;
    padding:2px 8px; background:var(--color-primary-lt); color:var(--color-primary);
    border:1px solid var(--color-primary); border-radius:20px;
}
.cds-pill-x { cursor:pointer; font-size:10px; opacity:.7; line-height:1; padding:0 1px; }
.cds-pill-x:hover { opacity:1; }
.cds-placeholder { font-size:12px; color:var(--color-text-faint); padding:2px 0; }
.cds-panel {
    display:none; position:absolute; left:0; right:0; top:calc(100% + 4px);
    z-index:6000; background:var(--color-card);
    border:1px solid var(--color-border); border-radius:var(--radius-md);
    box-shadow:var(--shadow-modal); overflow:hidden;
}
.cds-panel.open { display:block; animation:cdsIn .15s ease; }
@keyframes cdsIn { from{opacity:0;transform:translateY(-4px)} to{opacity:1;transform:translateY(0)} }
.cds-add-row {
...
.cds-add-input {
    ...
    background:var(--color-card); color:var(--color-text); transition:border-color .15s;
}
.cds-add-input:focus { outline:none; border-color:var(--color-primary); }
.cds-add-input.amt { width:72px; flex:none; }
.cds-add-btn {
    padding:5px 10px; border-radius:6px; font-size:12px; font-weight:600;
    background:var(--color-primary); color:#fff; border:none; cursor:pointer;
    white-space:nowrap; display:flex; align-items:center; gap:4px; transition:opacity .15s;
}
.cds-add-btn:hover { opacity:.88; }
.cds-add-err {
    padding:5px 10px; font-size:11px; color:#dc2626;
    background:#fee2e2; border-bottom:1px solid #fecaca;
    display:flex; align-items:center; gap:5px;
}
.cds-list { max-height:200px; overflow-y:auto; scrollbar-width:thin; }
.cds-item {
    display:flex; align-items:center; gap:8px; padding:7px 10px;
    cursor:pointer; transition:background .1s;
    border-bottom:1px solid var(--color-border); font-size:12px;
}
.cds-item:last-child { border-bottom:none; }
.cds-item:hover { background:var(--color-surface); }
.cds-item.selected { background:var(--color-primary-lt); }
.cds-item.is-opd { font-weight:600; }
.cds-check { width:18px; flex-shrink:0; color:var(--color-border); font-size:14px; }
.cds-item.selected .cds-check { color:var(--color-primary); }
.cds-name { flex:1; min-width:0; }
.cds-opd-badge {
    display:inline-block; font-size:9px; font-weight:700;
    padding:1px 5px; background:var(--color-accent-lt);
    color:var(--color-accent); border-radius:4px; margin-left:4px;
}
.cds-amount {
    font-family:'JetBrains Mono',monospace; font-size:11px; font-weight:700;
    color:var(--color-accent); white-space:nowrap; padding-right:2px;
}
.cds-action-btn {
    width:22px; height:22px; border:none; border-radius:5px;
    display:inline-flex; align-items:center; justify-content:center;
    font-size:10px; cursor:pointer; transition:opacity .1s; padding:0; flex-shrink:0;
}
.cds-edit-btn { background:#dbeafe; color:#2563eb; }
.cds-del-btn  { background:#fee2e2; color:#dc2626; margin-left:2px; }
.cds-action-btn:hover { opacity:.75; }
.cds-edit-row {
    display:flex; align-items:center; gap:6px; padding:5px 8px;
    background:#eff6ff; border-bottom:1px solid var(--color-border);
}
.cds-edit-name-i,.cds-edit-amt-i {
    font-size:12px; padding:3px 6px; border:1px solid #93c5fd;
    border-radius:5px; background:#fff; color:var(--color-text);
}
.cds-edit-name-i { flex:1; }
.cds-edit-amt-i  { width:70px; }
.cds-save-btn { padding:3px 8px; font-size:11px; font-weight:700; background:var(--color-primary); color:#fff; border:none; border-radius:5px; cursor:pointer; }
.cds-cancel-btn { padding:3px 7px; font-size:11px; background:var(--color-surface); color:var(--color-text); border:1px solid var(--color-border); border-radius:5px; cursor:pointer; }
.cds-total-row {
    display:flex; justify-content:space-between; align-items:center;
    padding:7px 10px; background:var(--color-surface);
    border-top:2px solid var(--color-border); font-size:12px; font-weight:700;
}
.cds-total-amt { font-family:'JetBrains Mono',monospace; font-size:13px; font-weight:800; color:var(--color-primary); }

/* ── Charge Selector — dark mode overrides ── */
.dark .cds-trigger { background:var(--color-card); }
.dark .cds-trigger:hover,
.dark .cds-trigger.open          { border-color:var(--color-primary); }
.dark .cds-panel                 { background:#1e293b; border-color:#334155; }
.dark .cds-add-row               { background:#263148; border-color:#334155; }
.dark .cds-add-input             { background:#1e293b; border-color:#334155; color:#e2e8f0; }
.dark .cds-add-input:focus       { border-color:var(--color-primary); }
.dark .cds-add-err               { background:rgb(220 38 38/.15); color:#f87171; border-color:rgb(220 38 38/.3); }
.dark .cds-item                  { border-color:#334155; color:#e2e8f0; }
.dark .cds-item:hover            { background:#263148; }
.dark .cds-item.selected         { background:rgb(3 105 161/.2); }
.dark .cds-item.selected .cds-check { color:#38bdf8; }
.dark .cds-item.selected .cds-name  { color:#e2e8f0; }
.dark .cds-check                 { color:#475569; }
.dark .cds-amount                { color:#34d399; }
.dark .cds-pill { background:rgb(3 105 161/.25); color:#38bdf8; border-color:#0284c7; }
.dark .cds-edit-row              { background:rgb(3 105 161/.12); border-color:#334155; }
.dark .cds-edit-name-i,
.dark .cds-edit-amt-i            { background:#1e293b; color:#e2e8f0; border-color:#475569; }
.dark .cds-edit-btn              { background:rgb(37 99 235/.2); color:#60a5fa; }
.dark .cds-del-btn               { background:rgb(220 38 38/.2); color:#f87171; }
.dark .cds-save-btn              { background:var(--color-primary); color:#fff; }
.dark .cds-cancel-btn            { background:#334155; color:#e2e8f0; border-color:#475569; }
.dark .cds-total-row             { background:#263148; border-color:#334155; color:#e2e8f0; }
.dark .cds-total-amt             { color:#38bdf8; }
</style>
...
<div class="stats-grid" id="statsGrid">
    ...
    <div class="stat-card sc-blue">
        ...
        <div class="stat-body">
            <div class="stat-value" id="statTotal"><div class="stat-skeleton" style="width:60px;"></div></div>
    ...
    <div class="stat-card sc-green">
        ...
        <div class="stat-body">
            <div class="stat-value" id="statRevenue"><div class="stat-skeleton" style="width:80px;"></div></div>
    ...
    <div class="stat-card sc-amber">
        ...
        <div class="stat-body">
            <div class="stat-value" id="statPending"><div class="stat-skeleton" style="width:80px;"></div></div>
    ...
    <div class="stat-card sc-slate">
        ...
        <div class="stat-body">
            <div class="stat-value" id="statReturned"><div class="stat-skeleton" style="width:40px;"></div></div>
        ...
        <div class="modal-body" style="padding:1rem;">
            ...
            <div class="gt-modal-body">
                ...
                <div class="gt-right-panel">
                    ...
                    <!-- ── Extra Charges ── -->
                    <div class="gt-section-label">
                        <i class="fa-solid fa-file-invoice-dollar"></i> Extra Charges
                    </div>
                    <input type="hidden" id="gtChargesJson" value="[]">
                    <div class="cds-wrapper mb-3" id="gtCdsWrapper">
                        <div class="cds-trigger" id="gtCdsTrigger" onclick="gtCds.toggle()">
                            <div id="gtCdsPills"><span class="cds-placeholder">Loading charges…</span></div>
                            <span class="cds-caret"><i class="fa-solid fa-chevron-down"></i></span>
                        </div>
                        <div class="cds-panel" id="gtCdsPanel">
                            <div class="cds-add-row">
                                <input type="text" class="cds-add-input" id="gtCdsNewName" placeholder="Charge name" autocomplete="off">
                                <input type="number" class="cds-add-input amt" id="gtCdsNewAmt" placeholder="Rs." min="0" step="0.01">
                                <button type="button" class="cds-add-btn" onclick="gtCds.addNew()">
                                    <i class="fa-solid fa-plus"></i> Add
                                </button>
                            </div>
                            <div class="cds-add-err" id="gtCdsAddErr" style="display:none;">
                                <i class="fa-solid fa-circle-xmark"></i><span></span>
                            </div>
                            <div class="cds-list" id="gtCdsList"></div>
                            <div class="cds-total-row">
                                <span><i class="fa-solid fa-sigma" style="font-size:10px;margin-right:3px;"></i>Total</span>
                                <span class="cds-total-amt" id="gtCdsTotal">Rs. 0</span>
                            </div>
                        </div>
                    </div>
...
(function () {
    ...
    // ════════════════════════════════════════════════════════════
    // CHARGE SELECTOR (gtCds)
    // ════════════════════════════════════════════════════════════
    window.gtCds = (function () {
        const IS_ADMIN   = window.APP_CONFIG.userRole === 'admin';
        let charges      = [];
        let selected     = new Set(); // ids
        let doctorFee    = 0;
        let editingId    = null;
        let isOpen       = false;
        const currSym    = window.APP_CONFIG.currencySymbol || 'Rs.';

        function fmt(n) { return currSym + ' ' + parseFloat(n).toLocaleString('en-PK', {minimumFractionDigits:2,maximumFractionDigits:2}); }

        function load() {
            fetch(`${AJAX_URL}?action=getCharges`)
                .then(r => r.json())
                .then(res => {
                    if (!res.success) return;
                    charges = res.data.charges;
                    // Auto-select OPD
                    charges.forEach(c => { if (c.is_opd) selected.add(c.id); });
                    render();
                });
        }

        function setDoctorFee(fee) {
            doctorFee = fee;
            charges.forEach(c => { if (c.is_opd) c._runtimeAmt = fee; });
            render();
            syncJson();
        }

        function getAmt(c) { return c.is_opd ? doctorFee : c.amount; }

        function getSelected() {
            return charges
                .filter(c => selected.has(c.id))
                .map(c => ({ name: c.name, amount: getAmt(c), is_opd: c.is_opd ? 1 : 0 }));
        }

        function syncJson() {
            document.getElementById('gtChargesJson').value = JSON.stringify(getSelected());
            const total = getSelected().reduce((s, c) => s + c.amount, 0);
            document.getElementById('gtCdsTotal').textContent = fmt(total);
            // Update pills
            renderPills();
        }

        function renderPills() {
            const box   = document.getElementById('gtCdsPills');
            const items = getSelected();
            if (!items.length) {
                box.innerHTML = '<span class="cds-placeholder">Select charges…</span>';
                return;
            }
            box.innerHTML = items.map(c =>
                `<span class="cds-pill">${escHtml(c.name)}
                    <span class="cds-pill-x" onclick="event.stopPropagation();gtCds.deselect(${charges.find(x=>x.name===c.name)?.id})">
        ...
        function render() {
            const list = document.getElementById('gtCdsList');
            if (!list) return;
            list.innerHTML = charges.map(c => {
                const amt    = getAmt(c);
                const isSel  = selected.has(c.id);
                const canDel = IS_ADMIN && !c.is_opd;

                if (editingId === c.id) {
                    return `<div class="cds-edit-row" id="citem-${c.id}">
                        <input class="cds-edit-name-i" id="ceN-${c.id}" value="${escHtml(c.name)}"
                               ${c.is_opd ? 'readonly' : ''} style="${c.is_opd?'opacity:.5;':''}" autocomplete="off">
                        <input class="cds-edit-amt-i" id="ceA-${c.id}" type="number" value="${c.amount}" min="0" step="0.01"
                               ${c.is_opd ? 'placeholder="Doctor fee"' : ''}>
                        <button class="cds-save-btn" onclick="gtCds.saveEdit(${c.id})"><i class="fa-solid fa-check"></i></button>
                        <button class="cds-cancel-btn" onclick="gtCds.cancelEdit()"><i class="fa-solid fa-xmark"></i></button>
                    </div>`;
                }

                return `<div class="cds-item${isSel?' selected':''}${c.is_opd?' is-opd':''}"
                             id="citem-${c.id}" onclick="gtCds.toggleItem(${c.id})">
                    <div class="cds-check">
                        <i class="fa-solid ${isSel?'fa-square-check':'fa-square'}"></i>
                    </div>
                    <div class="cds-name">
                        ${escHtml(c.name)}
                        ${c.is_opd ? '<span class="cds-opd-badge">Doctor Fee</span>' : ''}
                    </div>
                    <div class="cds-amount">${escHtml(fmt(amt))}</div>
                    ${canDel ? `
                    <button class="cds-action-btn cds-edit-btn" title="Edit charge"
                            onclick="event.stopPropagation();gtCds.startEdit(${c.id})">
                        <i class="fa-solid fa-pen"></i>
                    </button>
                    <button class="cds-action-btn cds-del-btn" title="Delete charge"
                            onclick="event.stopPropagation();gtCds.del(${c.id})">
        ...
        function toggle(id) {
            ...
            if (!c || c.is_opd) return;
            if (selected.has(id)) selected.delete(id); else selected.add(id);
            // Update only the clicked row in-place — full re-render would detach
            // e.target from the DOM, causing the document click-away listener to
            // wrongly close the panel.
            const el = document.getElementById(`citem-${id}`);
            if (el) {
                const isSel = selected.has(id);
                el.classList.toggle('selected', isSel);
                const icon = el.querySelector('.cds-check i');
                if (icon) icon.className = `fa-solid ${isSel ? 'fa-square-check' : 'fa-square'}`;
            }
            syncJson();
        }
        ...
        function addNew() {
            const nameEl = document.getElementById('gtCdsNewName');
            const amtEl  = document.getElementById('gtCdsNewAmt');
            const errEl  = document.getElementById('gtCdsAddErr');
            ...
            if (!name)           { showErr('Charge name is required.'); nameEl.focus(); return; }
            if (name.toLowerCase() === 'opd') { showErr('OPD is built-in and cannot be added again.'); return; }
            if (!amt || isNaN(parseFloat(amt)) || parseFloat(amt) < 0) {
                showErr('Please enter a valid amount (0 or more).'); amtEl.focus(); return;
            }
            // Client-side duplicate check
            const dup = charges.find(c => c.name.toLowerCase() === name.toLowerCase());
            if (dup) { showErr(`"${name}" already exists in the list.`); return; }

            const btn = document.getElementById('gtCdsAddErr').closest('.cds-panel')
                            .querySelector('.cds-add-btn');
            const origBtnHtml = btn.innerHTML;
            btn.innerHTML = '<span class="spinner" style="width:12px;height:12px;"></span>';
            btn.disabled  = true;

            const fd = new FormData();
            fd.set('action',     'addCharge');
            fd.set('csrf_token', getCsrfToken());
            fd.set('name',       name);
            fd.set('amount',     amt);
            ...
            fetch(AJAX_URL, { method:'POST', body:fd })
                ...
                .then(res => {
                    ...
                    charges.push(res.data.charge);
                    selected.add(res.data.charge.id);
                    nameEl.value = '';
                    amtEl.value  = '';
                    errEl.style.display = 'none';
                    render();
                ...
                .catch(() => { btn.innerHTML = origBtnHtml; btn.disabled = false; showErr('Network error.'); });
        }

        function startEdit(id) {
            editingId = id;
            render();
            const ni = document.getElementById(`ceN-${id}`);
            if (ni && !charges.find(c=>c.id===id)?.is_opd) ni.focus();
        }
        ...
        function saveEdit(id) {
            const c    = charges.find(x => x.id === id);
            if (!c) return;
            const name = (document.getElementById(`ceN-${id}`)?.value || c.name).trim();
            const amt  = parseFloat(document.getElementById(`ceA-${id}`)?.value ?? c.amount);

            if (!name) { showToast('Name cannot be empty.', 'error'); return; }
            if (isNaN(amt) || amt < 0) { showToast('Invalid amount.', 'error'); return; }

            // Duplicate check (excluding self)
            const dup = charges.find(x => x.name.toLowerCase() === name.toLowerCase() && x.id !== id);
            if (dup) { showToast(`"${name}" already exists.`, 'error'); return; }

            const fd = new FormData();
            fd.set('action',     'updateCharge');
            fd.set('csrf_token', getCsrfToken());
            fd.set('id',         id);
            fd.set('name',       name);
            fd.set('amount',     amt);
            ...
            fetch(AJAX_URL, { method:'POST', body:fd })
                ...
                .then(res => {
                    if (!res.success) { showToast(res.message, 'error'); return; }
                    c.name   = res.data.charge.name;
                    c.amount = res.data.charge.amount;
                    editingId = null;
                    render();
        ...
        function del(id) {
            ...
            showConfirmModal(
                'Remove Charge',
                `Remove charge "<strong>${escHtml(c.name)}</strong>"? It will no longer appear in new tokens.`,
                'Remove',
                () => {
                    const fd = new FormData();
                    fd.set('action',     'deleteCharge');
                    fd.set('csrf_token', getCsrfToken());
                    fd.set('id',         id);
                    fetch(AJAX_URL, { method:'POST', body:fd })
                        .then(r => r.json())
        ...
        function toggle_panel() {
            isOpen = !isOpen;
            document.getElementById('gtCdsTrigger').classList.toggle('open', isOpen);
            document.getElementById('gtCdsPanel').classList.toggle('open', isOpen);
            if (isOpen) document.getElementById('gtCdsNewName').focus();
        }
        ...
        function reset() {
            ...
            editingId = null;
            isOpen    = false;
            if (document.getElementById('gtCdsTrigger'))
                document.getElementById('gtCdsTrigger').classList.remove('open');
            if (document.getElementById('gtCdsPanel'))
                document.getElementById('gtCdsPanel').classList.remove('open');
            render();
        }

        // Close panel on outside click
        document.addEventListener('click', e => {
            if (isOpen && !document.getElementById('gtCdsWrapper')?.contains(e.target)) {
                isOpen = false;
                document.getElementById('gtCdsTrigger')?.classList.remove('open');
                document.getElementById('gtCdsPanel')?.classList.remove('open');
            }
        ...
        return { load, setDoctorFee, toggleItem: toggle, deselect, addNew, startEdit, cancelEdit, saveEdit, del, toggle: toggle_panel, reset };
    })();
    ...
    document.getElementById('openGenTokenBtn').addEventListener('click', () => {
        ...
        loadGtDoctors();
        if (!gtCds._loaded) { gtCds.load(); gtCds._loaded = true; }
        else gtCds.reset();
    });
    ...
    function refreshGtToken() {
        ...
        fetch(`${AJAX_URL}?${new URLSearchParams({ action:'getNextToken', doctor_id: gt.selectedDoctorId, visit_date: visitDate })}`)
            ...
            .then(res => {
                ...
                // Sync OPD amount in charge selector
                gtCds.setDoctorFee(res.data.fee);
    ...
    function submitGenToken() {
        ...
        fd.set('payment_status',  payStatus);
        fd.set('extra_charges_json', document.getElementById('gtChargesJson').value);
        if (payStatus === 'paid') fd.set('payment_method', method);
