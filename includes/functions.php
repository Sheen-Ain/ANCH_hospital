...
function renderReceiptHTML(int $patientId): string
{
    ...
    // ── Extra charges breakdown (falls back gracefully for old records) ──
    $patientCharges = Database::fetchAll(
        "SELECT name, amount, is_opd FROM patient_charges
     WHERE patient_id = ? ORDER BY is_opd DESC, sort_order ASC, id ASC",
        [$patientId]
    );
    $hasCharges       = !empty($patientCharges);
    $opdCharge        = null;
    $extraOnlyCharges = [];
    if ($hasCharges) {
        foreach ($patientCharges as $pc) {
            if ($pc['is_opd']) {
                $opdCharge = $pc;
            } else {
                $extraOnlyCharges[] = $pc;
...
?>
    ...
    <head>
        ...
        <style>
            ...
            /* ── Extra charges indent ── */
            #rct-root .info-tbl .lbl-indent {
                width: 45%;
                text-align: left;
                font-size: 11px;
                padding-left: 6px;
            ...
            <div id="rct">
                ...
                <table class="info-tbl">
                    <?php if ($hasCharges): ?>
                        ...
                        <?php if ($opdCharge): ?>
                            <tr>
                                <td class="lbl">OPD Fee</td>
                                <td class="val"><?= formatCurrency((float)$opdCharge['amount']) ?></td>
                            </tr>
                        <?php endif; ?>

                        <?php if (!empty($extraOnlyCharges)): ?>
                            <?php foreach ($extraOnlyCharges as $ec): ?>
                                <tr>
                                    <td class="lbl-indent">+ <?= e($ec['name']) ?></td>
                                    <td class="val"><?= formatCurrency((float)$ec['amount']) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td class="val-total"><?= $feeAmount ?> [<?= $payStatus ?>]</td>
                        </tr>
                    ...
                    <div class="urdu-wrap">
                        ...
                        <table class="urdu-tbl">
                            ...
                            <tr>
                                ...
                                <td class="u-lbl"><?= $hasCharges ? 'کل رقم' : 'فیس' ?></td>
                            </tr>
                            <tr>
                                <td class="u-val">
